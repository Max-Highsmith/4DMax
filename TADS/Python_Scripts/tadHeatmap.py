...
for c, chro in enumerate(CHROS):
	for t, (day, indx, mis) in enumerate(zip(['D2', 'D4', 'D6','D8'], [4,8,12,16], MISS)):
		...
		rightMis  = mis + 2
		if mis !=8:
			bar[c, t,1] = compfunc(tads[chro, rightMis][indx], base)
		else:
			bar[c, t,1] = compfunc(tads[chro, leftMis][indx], base)
		bar[c,t,2] = compfunc(tads[chro, mis][indx], base)
		#left and right neighbors
		#bar[c,t,4]  = compfunc(real_TAD[chro,1,leftDay], base)
		#bar[c,t,5]  = compfunc(real_TAD[chro,1, rightDay], base)
...
for c, chro in enumerate(CHROS):
	...
	for (j, i), label in np.ndenumerate(bar[c]):
		...
		#ax[c].set_xticks([])
		ax[int(c%5),int(c/5)].text(i,j, "{0:0.2f}".format(bar[c,j,i]), ha='center', va='center', fontsize="xx-small", color="white")
...
ax[4,3].set_xticklabels(["HiC Rep", "4D Recon", "4D Interp", "Left Map", "Right Map"])
