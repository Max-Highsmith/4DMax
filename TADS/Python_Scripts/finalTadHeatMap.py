...
for c, chro in enumerate(CHROS):
    for t, (day, indx, mis) in enumerate(
        ...
        rightMis  = mis + 2
        if mis !=8:
                bar[c, t,1] = compfunc(tads[chro,
                    rightMis,1][indx], base)
                bar[c, t,3] = compfunc(tads[chro,
                    rightMis,1][indx], real_TAD[chro,2,day])
        else:
                bar[c, t,1] = compfunc(tads[chro,
                    leftMis,1][indx], base)
                bar[c, t,3] = compfunc(tads[chro,
                    leftMis,1][indx], real_TAD[chro,2,day])
...
for c, chro in enumerate(CHROS):
	...
	for (j, i), label in np.ndenumerate(bar[c]):
		...
		#ax[c].text(i,j, "{0:0.2f}".format(bar[c,j,i]),
                    #ha='center',
                    #va='center',
                    #fontsize="xx-small",
                    #color="white")
